...
import { type SustainabilityAction, createAction, updateAction, formatApiError } from "@/lib/api"
...
export function ActionForm({ action, onSave, onCancel }: ActionFormProps) {
   ...
   */
  const handleSuggestionSelect = (suggestion: { action: string; points: number }) => {
    setFormData((prev) => ({
      ...prev,
      ...
      <DialogContent className="backdrop-blur-md bg-card/80 border border-border/20 shadow-xl max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-6">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <Textarea
                ...
                className={`backdrop-blur-md bg-card/60 border border-border/20 min-h-[80px] resize-none ${errors.action ? "border-destructive" : ""}`}
                maxLength={255}
              />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className={`backdrop-blur-md bg-card/60 border border-border/20 ${errors.date ? "border-destructive" : ""}`}
              />
            ...
            {/* Points Field */}
            <div className="space-y-2">
              <Label htmlFor="points" className="text-sm font-medium flex items-center gap-1">
              ...
              <div className="relative">
                <Input
                  ...
                  className={`backdrop-blur-md bg-card/60 border border-border/20 ${errors.points ? "border-destructive" : ""}`}
                />
                <div
                  className={`absolute right-3 top-1/2 -translate-y-1/2 text-sm font-medium ${getPointsColor(formData.points)}`}
                >
                  +{formData.points}
                </div>
              </div>
          ...
          <div className="backdrop-blur-md bg-card/60 border border-border/20 shadow-lg rounded-lg p-4">
            <h4 className="text-sm font-medium text-foreground mb-2">Points Guide</h4>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-muted rounded-full" />
                <span className="text-muted-foreground">Small actions: 10-25 pts</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-secondary rounded-full" />
                <span className="text-muted-foreground">Medium actions: 25-50 pts</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-primary rounded-full" />
                <span className="text-muted-foreground">Major actions: 50+ pts</span>
              </div>
            </div>
          </div>
